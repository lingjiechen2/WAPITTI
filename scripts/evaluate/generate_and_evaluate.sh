...
MODEL_BASE_DIR="meta-llama/Llama-2-7b-hf-logit-watermark-distill-kgw-k0-gamma0.25-delta1"
models=($(ls -d "$TRAINED_MODEL_PATH_PREFIX/$MODEL_BASE_DIR"/checkpoint-* | sort -V))
printf '%s\n' "${models[@]}"
base_name=$(basename "$MODEL_BASE_DIR")
model_family="${MODEL_BASE_DIR%%/*}"
output_dir="/mnt/petrelfs/fanyuyu/fyy/WAPITI-Code-Base-master/results/watemmark_distillation/${model_family}/${base_name}"
...
for model in "${models[@]}"; do
    ...
    (    
        ...
        python experiments/compute_metrics.py \
            ...
            --watermark_tokenizer_name "${model}" \
            --truncate \
