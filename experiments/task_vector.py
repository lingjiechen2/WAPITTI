            ...
            with torch.no_grad():
                ...
                for key in pretrained_state_dict:
                    ...
                    try:
                        ...
                        print(f'Warning: key {key} has a different size in the pretrained and finetuned model. The shape of the pretrained model is {pretrained_state_dict[key].shape} and the shape of the finetuned model is {finetuned_state_dict[key].shape}')
                        self.vector[key] = torch.zeros_like(pretrained_state_dict[key])
                        continue        
        ...
        with torch.no_grad():
            ...
            for key in pretrained_state_dict:
                ...
                try:
                    ...
                    print(f'Warning: key {key} has a different size in the task vector and the pretrained model. The shape of the task vector is {self.vector[key].shape} and the shape of the pretrained model is {pretrained_state_dict[key].shape}')
                    continue
