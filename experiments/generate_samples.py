...
def get_prompts(args) -> Dict:
    ...
    dataset = dataset.map(encode,remove_columns=["url", "timestamp"], batched=False)
    ...
    for batch in dataloader:
        ...
        if (type(batch["attention_mask"]) == list):
            batch["attention_mask"] = torch.cat(batch["attention_mask"], dim=0).to(device)  # (B, 50)
        batch["attention_mask"] = batch["attention_mask"].squeeze()
        batch["input_ids"] = batch["input_ids"].squeeze()
        prompts.append(batch)
        human_text.extend(list(batch["text_completion"][0]))
        prompt_text.extend(list(batch["prompt_text"][0]))
        full_human_text.extend(list(batch["text"][0]))
